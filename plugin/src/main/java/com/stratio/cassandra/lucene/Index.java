...
public class Index implements org.apache.cassandra.index.Index {
    ...
    public static Map<String, String> validateOptions(Map<String, String> options, CFMetaData metaData){
        ...
        try {
            ...
            LOGGER.error(MessageFormatter.format("Invalid index options: {} ", options).getMessage(), e);
    ...
    private static IndexService buildService(ColumnFamilyStore table, IndexMetadata indexMetadata){
        LOGGER.debug("Building Lucene index {} {}", table.metadata, indexMetadata);
        try {
            return IndexServiceBuilder.build(table, indexMetadata);
        } catch (Exception e){
    ...
    public final IndexService service;
    ...
    public Index(ColumnFamilyStore table, IndexMetadata indexMetadata) {
        ...
        this.service = buildService(table, indexMetadata);
    ...
    public Callable<?> getMetadataReloadTask(IndexMetadata indexMetadata) {
        ...
        if(!indexMetadata.equals(this.indexMetadata)){
            return this::getBuildIndexTask;
        } else {
            return () -> null;
        }
    ...
    public boolean dependsOn(ColumnDefinition column) {
        ...
        LOGGER.trace("Asking if the index depends on column {}", column);
    ...
    public boolean supportsExpression(ColumnDefinition column, Operator operator) {
        LOGGER.trace("Asking if the index supports the expression {} {}", column, operator);
        return service.expressionMapper.supports(column, operator);
    }
    ...
    public RowFilter getPostIndexQueryFilter(RowFilter filter) {
        ...
        return service.expressionMapper.postIndexQueryFilter(filter);
    }
    ...
    public void validate(PartitionUpdate update) throws InvalidRequestException {
        ...
        try {
            ...
            LOGGER.debug(MessageFormatter.format("Invalid partition update: {}", update).getMessage(), e);
    ...
    public Indexer indexerFor(DecoratedKey key,
        ...
        return service.writer(key, nowInSec, opGroup, transactionType);
    }
    ...
    public BiFunction<PartitionIterator, ReadCommand, PartitionIterator> postProcessorFor(ReadCommand command) {
        return new IndexPostProcessor.ReadCommandPostProcessor(service);
    }
    ...
    public BiFunction<PartitionIterator, SinglePartitionReadCommand.Group, PartitionIterator> postProcessorFor(SinglePartitionReadCommand.Group group) {
        return new IndexPostProcessor.GroupPostProcessor(service);
    }
    ...
    public Searcher searcherFor(ReadCommand command) {
        ...
        try {
            return executionController -> service.search(command, executionController);
        } catch (Exception e){
            LOGGER.error(MessageFormatter.format("Error getting searcher for command: {}", command).getMessage(), e);
    ...
    public Search validate(RowFilter.Expression expression){
        try {
           return service.validate(expression);
            ...
            LOGGER.debug(MessageFormatter.format("Invalid index expression: {}", expression).getMessage(), e);
            throw new InvalidRequestException(e.getMessage());
        }
    }
